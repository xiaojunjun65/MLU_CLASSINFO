...
class OfflineTester:
        ...
        with open(self.ops.dataset_path) as f:
            ...
            for info in allImageFileInfo:
                if ',' in info.strip():
                    ...
                    self.label.append(self.label2num[label])
                else:
                    self.image_list.append(info.strip())
                    self.complieAcc = False
    ...
    def generateQuantizeModel(self):
        ...
        for image_file in self.image_list[:self.ops.iteration]:
            img = Image.open(os.path.join(self.basePath, image_file))
    ...
    def testOfflineModel(self):
        ...
        for image_file in self.image_list:
            image = Image.open(os.path.join(self.basePath, image_file))
            ...
            with torch.no_grad():
                ...
                predictLabe.append(res)
                fp.write('{},{}:{:.5f}\n'.format(image_file, self.num2label[res], outputs[0][res]))
        fp.close()
        if self.complieAcc:
            err = np.count_nonzero(np.array(self.label) - np.array(predictLabe))
            count = len(predictLabe)
            acc = (count - err) / count
            info = {"info":{"accuracy":acc}}
